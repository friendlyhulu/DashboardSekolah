...
<?php
//Menampilkan data dari id yang dipilih
$id_siswa = $_GET['id_siswa'];

$query = "SELECT * FROM tabel_siswa where id_siswa=$id_siswa";
...
while ($user_data = mysqli_fetch_array($hasil)){
    ...
    $ekskul         = $user_data["ekskul"];  

}
?>

<?php
//update data ke tabel siswa
if(isset($_POST['update'])){
    ...
    $query = "UPDATE tabel_siswa SET nis ='$nis', nama_siswa ='$nama_siswa', jenis_kelamin ='$jenis_kelamin',
    tempat_lahir ='$tempat_lahir', tanggal_lahir ='$tanggal_lahir', alamat ='$alamat', no_hp='$no_hp', ekskul='$ekskul' where id_siswa = $id_siswa";
    $hasil = mysqli_query($con, $query);

    //Notikasi ketika berhasil di tambahkan
    if ($hasil) {
        echo '<br><div class="alert alert-primary" role="alert">Data siswa berhasil diperbaharui- <a href="./tampil_siswa.php" class="alert-link">Lihat Data Siswa</a></div>';
    } else {
        echo '<br><div class="alert alert-danger" role="alert">Data siswa gagal diperbaharui, mohon cek kembali</div>';
    }
...
<!-- Content --> 
    ...
    <div class="card text-white bg-dark mb-3"> &ensp;
        <h4 style="padding-left: 15px;"><b>  Edit Siswa</b></h4>
            <div class="card-header">Silahkan lengkapi form berikut untuk memperbaharui data siswa/i</div>
        </div>
    ...
    <form action = "update_siswa.php?id_siswa=<?php echo $id_siswa; ?>" method = "post" id="formtambah">
        ...
        <option selected><?php echo $ekskul;?></option>
